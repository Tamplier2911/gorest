...
func TestGoogleAuth(t *testing.T) {
	...
	t.Run("should redirect to google popup", func(t *testing.T) {
		var res testclient.DefaultResponse
		err := testClient.Request(&testclient.RequestOptions{
			Method:          "GET",
			URL:             "/api/v2/auth/google/login",
			DefaultResponse: &res,
		})
	...
	t.Run("should perform callback logic", func(t *testing.T) {
		var res auth.GoogleCallbackHandlerResponseBody
		err := testClient.Request(&testclient.RequestOptions{
			Method:   "GET",
			URL:      fmt.Sprintf("/api/v2/auth/google/callback?state=%s", a.Config.GoogleClientState),
			Response: &res,
		})
		require.NoError(t, err, "parsed invalid uuid")
	...
	t.Run("user should be created in database", func(t *testing.T) {
		...
		err := a.MySQL.
			Model(&models.User{}).
			Where(&models.User{Email: googleFixtures.GoogleUser.Email}).
			First(&user).
			Error
		require.NoError(t, err, "parsed invalid uuid")
		require.Equal(t, user.UserRole, models.UserRoleUser, "unexpected user role")
		require.Equal(t, user.AvatarURL, googleFixtures.GoogleUser.Picture, "unexpected avatar url")
		require.Equal(t, user.Email, googleFixtures.GoogleUser.Email, "unexpected email address")
		testUserId = user.ID
	})
	...
	t.Run("auth provider should be created in database", func(t *testing.T) {
		...
		err := a.MySQL.
			...
			Where(&models.AuthProvider{ProviderUID: googleFixtures.GoogleUser.ID}).
			First(&provider).
			Error
		require.NoError(t, err, "parsed invalid uuid")
		require.Equal(t, provider.AuthProviderType, models.AuthProviderTypeGoogle, "unexpected provider type")
		require.Equal(t, provider.ProviderUID, googleFixtures.GoogleUser.ID, "unexpected provider uid")
		require.Equal(t, provider.RefreshToken, googleFixtures.Token.RefreshToken, "unexpected refresh token value")
	})
